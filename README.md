# Hi there, I'm Advay Sharma 👋

### 🚀 Aspiring Software Engineer | Java Developer | DSA Enthusiast | AI & RAG Learner

I'm a Computer Science student passionate about software development, problem-solving, and building impactful projects. I enjoy learning new technologies, creating real-world applications, and continuously improving my technical skills.

---

## 👨‍💻 About Me

- 🎓 Computer Science Student
- 💻 Passionate about Software Development and Problem Solving
- 🌱 Currently learning Full Stack Development, System Design, and RAG (Retrieval-Augmented Generation)
- 🧠 Regularly practicing Data Structures & Algorithms
- 🤖 Exploring AI-powered applications and modern web technologies
- 🎯 Looking for internship and collaborative project opportunities

---

## 🛠️ Tech Stack

### Languages
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge)

### Web Development
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)

### Tools & Technologies
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-02569B?style=for-the-badge)

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&theme=tokyonight&show_icons=true&hide_border=true&count_private=true)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=YOUR_GITHUB_USERNAME&theme=tokyonight&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&theme=tokyonight&hide_border=true&layout=compact)

---

## 🌱 Currently Learning

- Advanced Data Structures & Algorithms
- Full Stack Web Development
- Retrieval-Augmented Generation (RAG)
- Generative AI Applications
- System Design Fundamentals

---

## 🚀 Featured Projects

### 🔹 CodeLink
A platform that helps developers connect, collaborate, and share resources.

**Tech Stack:** HTML, CSS, JavaScript, React, Node.js

### 🔹 RAG AI Assistant
An AI-powered application that retrieves relevant information and generates context-aware responses.

**Tech Stack:** Python, LangChain, Vector Database, OpenAI APIs

---

## 🏆 Achievements

- Solved 500+ DSA problems across coding platforms
- Built multiple full-stack web applications
- Participated in coding contests and hackathons
- Continuously learning and building projects

---

## 🎯 2026 Goals

- Solve 1000+ DSA Problems
- Contribute to Open Source Projects
- Secure a Software Development Internship
- Build More Full-Stack Applications
- Deepen Knowledge in AI and RAG Systems

---

## 📫 Connect With Me

- 📧 Email: advaysharma2489@gmail.com
- 💼 LinkedIn: https://github.com/AdvaySharma248
- 💻 LeetCode: https://leetcode.com/u/Advay_Sharma/

---

### ⭐ Thanks for visiting my profile!

> "Code. Learn. Build. Repeat."
